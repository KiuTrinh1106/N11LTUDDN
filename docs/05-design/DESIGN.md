...
| US-AI-09                              | Tìm kiếm tài liệu theo quyền                 | `Search`, `Search Results`, `Search / Empty`, `Search / Error`, `Search / Permission Denied` | Chỉ hiển thị kết quả được phép; không tiết lộ metadata tài liệu ngoài quyền và không có Internet search.                                                                                                                                    |
| US-AI-10                              | Hỏi AI về tài liệu được phép                 | `Ask AI`, `AI Processing`, `AI Answer`, `AI / Insufficient Data`, `AI / Error`          | Reader submit câu hỏi bằng văn bản; AI chỉ dùng nguồn nội bộ được phép và version hiện hành; không suy đoán hoặc dùng Internet.                                                  |
| US-AI-11                              | Hiển thị và lưu citation                     | `Citation / Inline`, `Citation / Expanded`, `Citation / Unavailable`                    | Câu trả lời hiển thị và lưu document ID/version ID hoặc citation tương đương; chỉ mở nguồn mà Reader được phép xem.                                                            |
| US-AI-12                              | Ghi audit log hoạt động quan trọng           | `Audit History`, `Audit History / Loading`, `Audit History / Empty`, `Audit History / Error` | Ghi và hiển thị lịch sử Create, Edit, Review, Approve, Publish và Ask AI theo quyền; mỗi bản ghi có actor, action, object, timestamp và result.                                |

### 3.1 KEY SCREEN INVENTORY

| Màn hình chính | Role sử dụng | Mục đích | States tối thiểu cần thiết |
| -------------- | ------------ | ------- | --------------------------- |
| `Create Document` / `Document Editor` | Author | Tạo và chỉnh sửa tài liệu với Name, Content, Folder và Tag. | `default`, `loading`, `validation error`, `saving`, `success`, `error` |
| `Document Detail / Draft` | Author | Xem Draft, chỉnh sửa hoặc gửi Draft vào Review. | `draft`, `loading`, `confirmation`, `success`, `error`, `permission denied` |
| `Review Queue` | Reviewer, Admin | Xem các Review Request và thông tin Document, Version, Sender, Status. | `loading`, `empty`, `Reviewing`, `error`, `permission denied` |
| `Review Detail` | Reviewer, Admin | Kiểm tra Draft và thực hiện Approve hoặc Reject. | `Reviewing`, `confirmation`, `submitting`, `Approved`, `Rejected`, `error` |
| `Approved Detail` / `Published Detail` | Reviewer, Admin, Reader | Publish bản đã Approved hoặc xem version hiện hành đã Published. | `Approved`, `confirmation`, `loading`, `Published`, `error`, `permission denied` |
| `Version History` | Admin, Reviewer, Author, Reader | Xem version theo quyền và phân biệt current với superseded version. | `loading`, `empty`, `current`, `superseded`, `error`, `permission denied` |
| `Search` / `Search Results` | Reader | Tìm và mở tài liệu trong phạm vi quyền hiện tại. | `idle`, `typing`, `loading`, `results`, `no result`, `error`, `permission denied` |
| `Document Detail / Current Version` | Reader | Đọc tài liệu được phép và mở Ask AI. | `loading`, `success`, `error`, `permission denied` |
| `Ask AI` / `AI Answer` | Reader | Đặt câu hỏi bằng văn bản và kiểm tra câu trả lời cùng citation. | `empty`, `validation error`, `AI processing`, `answer`, `insufficient data`, `error` |
| `Audit History` | Admin | Truy vết hoạt động quan trọng của tài liệu và hệ thống. | `loading`, `results`, `empty`, `error`, `permission denied` |
