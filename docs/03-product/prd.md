...
| Publish có điều kiện                  | Chỉ Reviewer hoặc Admin được Publish version đã được Reviewer hoặc Admin Approve; từ chối version chưa hoàn tất Review, bị Reject hoặc chưa được Approve. Author không được Publish.                                                                      | REQ-FR-10, REQ-BR-01             | Must                             |
| Quản lý version hiện hành             | Khi nội dung cập nhật, tạo version mới và đánh dấu version hiện hành; version bị thay thế hoặc bị Reject được giữ nguyên, không sửa trực tiếp hoặc dùng làm version hiện hành. Author chỉnh sửa version bị Reject phải tạo version mới để gửi Review lại. | REQ-FR-11, REQ-BR-03, REQ-BR-05  | Must                             |
...
| ----- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| BR-01 | Chỉ version đã qua Review và được Reviewer hoặc Admin Approve mới được chuyển sang `Published`; version bị Reject hoặc chưa Approve không được Publish. Chỉ Reviewer hoặc Admin được Publish; Author không được Publish. (REQ-FR-10, REQ-BR-01)    |
...
| BR-03 | Khi nội dung được cập nhật, hệ thống phải tạo version mới; version bị thay thế hoặc bị Reject phải được giữ nguyên và không được coi là version hiện hành. Author chỉnh sửa version bị Reject phải tạo version mới để gửi Review lại. (REQ-FR-11)  |
...
1. Có yêu cầu nào mâu thuẫn hoặc chưa thể viết test case không?
   ...
   - Một số yêu cầu chưa thể viết test case hoàn chỉnh nếu chưa có quyết định bổ sung: mô hình và quy trình cấp quyền; quy tắc tăng version và version chính thức; phạm vi benchmark Q&A; danh sách hoạt động quan trọng; cơ chế xác nhận và thời điểm chạy xóa retention.
...
2. Ba câu hỏi quan trọng nhất cần con người quyết định:
   ...
   3. Bộ benchmark Q&A gồm bao nhiêu câu, ai phê duyệt đáp án và nguồn chuẩn, và thế nào là câu trả lời do đoán?
