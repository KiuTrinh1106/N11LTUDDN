...
Các flow tuân thủ workflow: `Create -> Review -> Approve -> Publish -> Version -> Search/Ask`. `Approved` là kết quả Review trung gian, chưa phải version chính thức; chỉ Reviewer hoặc Admin được `Publish` bản Draft đã `Approved`. Author không được `Approve` hoặc `Publish`. Search và Ask AI chỉ sử dụng tài liệu, metadata và version mà người dùng hiện tại có quyền truy cập.
...
flowchart TD
	...
	A11 --> AEnd((End))
	A6 -.-> ALog[AuditEvent: Create/Edit]
	A10 -.-> AReviewLog[AuditEvent: Review]
...
flowchart TD
	...
	B11 --> B13[Hiển thị action Publish]
	...
	B21 --> BEnd((End))
	B10 -.-> BApproveLog[AuditEvent: Approve]
	B15 -.-> BPublishLog[AuditEvent: Publish]
	B18 -.-> BRejectLog[AuditEvent: Reject]
	...
	class BE,BE1,BE2,BE3,BDenied error;
...
| Review Queue Item                   | Reviewer/Admin chọn tài liệu.                           | Mở Detail với document, version, Author, Folder, Tag và trạng thái.                                                                                                          | Nếu role không hợp lệ hoặc truy cập trực tiếp không có quyền, hiển thị `Permission denied` và không cho thao tác; nếu hợp lệ, cho phép `Approve` hoặc `Reject`. |
| Document Detail - Reviewing         | Người xử lý kiểm tra nội dung và metadata.              | Hiển thị đúng bản Draft đang được Review; lưu context tài liệu và version đang xử lý.                                                                                        | Chọn `Approve` để mở confirmation Approve hoặc `Reject` để nhập lý do Reject bắt buộc.                                                            |
...
| Confirmation - Reject               | Reviewer/Admin nhập lý do bắt buộc và xác nhận `Reject`. | Hiển thị `Loading`; lưu người xử lý, thời điểm, kết quả và lý do Reject.                                                                                                     | Lỗi hiển thị `Error`; thành công chuyển sang `Rejected`.                                                                                         |
...
flowchart TD
	...
	C4 --> C5{Chọn tài liệu?}
	C5 -->|Truy cập trực tiếp ngoài quyền| CDenied[Permission denied / không tiết lộ metadata]
	CDenied --> CEmpty
...
| Document Detail - current version   | Reader mở tài liệu được phép và chọn `Ask AI`.                            | Hiển thị nội dung và version hiện hành đã Published; không cho Reader dùng version bị thay thế làm nguồn Q&A.                               | Mở AI Question Form.                                                                                                                        |
...
- `Approved` là trạng thái trung gian sau khi Reviewer hoặc Admin Approve; bản này đã được chấp nhận nhưng chưa phải version chính thức.
...
- `Reject` phải lưu lý do, giữ bản Draft để Author chỉnh sửa và gửi lại trên cùng bản; `Reject` không tạo version chính thức.
...
## 6. Background flow - Retention version

Retention là tác vụ hệ thống chạy hằng ngày lúc 02:00 theo múi giờ hệ thống, không yêu cầu người dùng thao tác:

1. Hệ thống tìm các version không hiện hành đã đủ 1 năm kể từ `replaced_at`.
2. Hệ thống xóa các version đủ điều kiện.
3. Hệ thống luôn giữ lại version hiện hành và không xóa version chưa đủ thời hạn.
4. Kết quả tác vụ phải được ghi vào audit log để có thể truy vết thành công hoặc thất bại.
